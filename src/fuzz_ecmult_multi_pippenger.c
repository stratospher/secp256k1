 ...
 *   secp256k1_ecmult_pippenger_batch  vs  secp256k1_ecmult_multi_simple_var
 *
 * Build / run instructions match fuzz_ecmult.c — see CMakeLists.txt
 * (target: fuzz_ecmult_multi_pippenger).
 */
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    ret_test = secp256k1_ecmult_pippenger_batch(&err_cb, scratch, &r_test,
