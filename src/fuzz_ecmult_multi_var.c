 ...
 *   secp256k1_ecmult_multi_var  vs  secp256k1_ecmult_multi_simple_var
 *
 * Covers the dispatcher: algorithm selection, batching loop,
 * G-once-only invariant, and cb_offset arithmetic.
 *
 * Build / run instructions match fuzz_ecmult.c — see CMakeLists.txt
 * (target: fuzz_ecmult_multi_var).
 */
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    ret_test = secp256k1_ecmult_multi_var(&err_cb, scratch, &r_test,
